...
const Card = ({ element, styleOverrideClass, handleSubClick, handleFavClick, subscriptions }) => {
  ...
  return (
    <div className={`card ${element.id}`} style={{ backgroundImage: `url(${element.imageUrl})` }} >
          <article className={`card-content ${styleOverrideClass}`} key={cardState.id} >
              ...
              <div className='btn-container'>
                ...
                <button className='btn urlLink' onClick={() => window.open(element.url)}>link to {cardState.type.substring(0, cardState.type.length - 1)}</button>
                <button className={subscriptions[cardState.newsSite] ? 'btn active sub' : 'btn inactive sub'} onClick={() => handleSubClick( cardState )} >{subscriptions[cardState.newsSite] ? `unsubscribe from ${cardState.newsSite}` : `subscribe to ${cardState.newsSite}`}</button>
              </div>
          </article>
        </div>
  )
